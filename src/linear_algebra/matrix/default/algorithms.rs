...
use crate::linear_algebra::matrix::default::Matrix;
use crate::linear_algebra::error::{LinearAlgebraError, ErrorKind};
use num::{One, Zero, Float};
use std::ops::{MulAssign, DivAssign, Div, Sub, Add, Mul};
...
macro_rules! impl_lu_decomp_for_matrix
{
    ($id:ty) => 
    {
        impl<'a,T: Copy> PLUDecomposition for $id
        where
            T: Float
            + std::fmt::Debug
        {
...
impl_lu_decomp_for_matrix!(Matrix<T>);
impl_lu_decomp_for_matrix!(&'a Matrix<T>);
impl_lu_decomp_for_matrix!(&'a mut Matrix<T>);

/***** Eigen Value Decomposition ******/

impl<T> Norm<T> for Vec<T>
where
    T: Float
{
    fn eucl_norm(self) -> T
...
impl<'a, T: Copy> Balance for Matrix<T>
where
    T: Float
    + DivAssign<T>
    + MulAssign<T>
    ...
    fn balance(self) -> Self::Output 
    {
        ...
        let dim: usize = { 
            if worker.row != worker.col { 
                return Err(LinearAlgebraError::from(ErrorKind::MatrixStructure)) 
            } else { worker.row }
        };
