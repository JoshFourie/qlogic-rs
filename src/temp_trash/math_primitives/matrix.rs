...
use num::Complex;
use super::{ QuantumUnit, QuantumReal, ComplexAlgebra, VectorAlgebra, MatrixAlgebra };
...
impl<T:QuantumUnit> MatrixAlgebra<T> for Matrix<T>
{
    ...
    fn set(&mut self, row: usize, col: usize, val: T) -> Result<(), Self::Error>
    {
        ...
        {
            true => {
                self.inner[row*self.row+col] = val;
                Ok(())
            },
            false => MatrixError::invalid_index(row, col, self.dim).as_result(),
        }
    ...
    fn kronecker(&self, rhs: &Self) -> Result<Self,Self::Error>
    {
        ...
        let p =rhs.row as f64;
        let q = rhs.col as f64;
        for i in 1..=self.row {
            let i = i as f64;
            for j in 1..=self.col {
                let j = j as f64;              
                ...
                let b = rhs.get(
                    ((i-1.0).rem(p)+1.0) as usize -1,
                    ((j-1.0).rem(q)+1.0) as usize -1,
                )?;
                new.push(a*b);
            }
        }
        new.dim=dim;
        new.col = self.col.mul(rhs.col);
        new.row = self.row.mul(rhs.row);
        Ok(new)
    }
    ...
    fn cross(&self, rhs: &Self) -> Result<Self,Self::Error>
    {
        ...
        let new: Vec<T> = vec![T::zero(); self.dim()];
        for i in 0..self.row {
            for j in 0..rhs.col {
    ...
    {
        let mut id: Self = vec![T::zero(); self.col.mul(self.row)].into();
        for j in 0..self.dim {
            id.set(j,j,T::one())?
        }
        Ok(id)
...
impl<T:QuantumUnit> ComplexAlgebra<T> for ComplexMatrix<T>
where
